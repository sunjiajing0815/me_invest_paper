...
9. **LangGraph version pinning + undeclared runtime deps.** Pin `langgraph` to a specific minor version (`langchain-core` comes transitively); the ecosystem is historically version-churn-prone. The graph-integration test in `test_news_triage.py` is the canary after any upgrade. (`langchain-anthropic` and `langgraph-checkpoint-sqlite` were removed 2026-07 — never imported; LLM calls go through `services/llm.py`, checkpointing uses `MemorySaver`.) Separately, two pyproject deps exist ONLY as undeclared runtime requirements of other packages — `pytz` (imported by `alpaca-py`) and `opentelemetry-api` (imported by `claude-agent-sdk`); don't remove them even though nothing in `src/` imports them.
...
29. **`targets.yaml` must satisfy `band_low ≤ pct ≤ band_high` per ticker.** `load_targets()` in `config.py` enforces it (alongside the pct-sum check) and raises on violation. A target whose `pct` sits outside its own band loaded fine pre-`1cec2db` and made the holding read **perpetually "under band"** (e.g. QQQ `pct: 25` with `band: [26, 34]`). When editing a target's `pct`, move its `band` to bracket it. See post-4.9a §10.
